...
async function fetchOpenSkill(username, isAuthenticated) {
    ...
    if (openskillCache[username] !== undefined) {
        return openskillCache[username];
    }
    try {
        const response = await fetch(url);
        if (!response.ok) throw new Error('Network response was not ok');
        const data = await response.json();
        openskillCache[username] = data.openskill;
        return data.openskill;
    } catch (error) {
        console.error('Error fetching open skill for', username, ':', error);
        openskillCache[username] = null;
        return null;
    }
}
...
var settings = tpul.settings.addSettings({
    ...
    fields: {
        ...
        order: {
            ...
            type: 'select',
            options: ['constant','alphabetic','score','skill'],
            default: 'constant'
        },
        show_hold: {
    ...
    tagpro.ready(function () {
        ...
        function drawPlayer(player) {
            ...
            if (show_name) {
                ...
                if (openskillCache[player.name] === undefined) {
                    fetchOpenSkill(player.name, player.auth).then(openskill => {
                        if (openskill !== null) {
                            var openskillText = new PIXI.Text(` (${openskill.toFixed(2)})`, style[player.team]);
                            //openskillText.x = name.x + name.width + 5;
                            //openskillText.y = name.y;
                            //player.monitor.addChild(openskillText);
                            if (preset[player.team].leftText) {
                                // Position to the left of the ball icon
                                openskillText.x = textHLshift - openskillText.width - name.width - 2;
                            } else {
                                // Position to the right of the player name
                                // Find out the width of the name text to position the openskill text correctly
                                //const nameWidth = player.monitor.children.find(child => child instanceof PIXI.Text)?.width || 0;
                                openskillText.x = textHRshift + name.width;
                            }
                            openskillText.y = textVshift;
                            player.monitor.addChild(openskillText);
                        }
                    });
                } else if (openskillCache[player.name] !== null) {
                    const openskill = openskillCache[player.name];
                    var openskillText = new PIXI.Text(` (${openskill.toFixed(2)})`, style[player.team]);
                    //openskillText.x = name.x + name.width + 5;
                    //openskillText.y = name.y;
                    if (preset[player.team].leftText) {
            ...
            if (show_flag && player.flag && !player.dead) {
                ...
                if (show_hold) {
                    ...
                    if (preset[player.team].leftText)   hold.x = textHLshift - hold.width - (name ? name.width + openskillText.width:0);
                    else                                hold.x = textHRshift + (name ? name.width + openskillText.width:0);
            ...
            switch (order) {
                ...
                case 'skill':
                        teamPlayers.sort( (p1,p2) => {
                            const skill1 = openskillCache[p1.name] || 0;
                            const skill2 = openskillCache[p2.name] || 0;
